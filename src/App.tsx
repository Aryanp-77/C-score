...
import './App.css';
import { ALL_QUESTIONS, CATEGORIES, BRAND, UI, Question } from './config';
import {
...
function QuestionCard({
  ...
  return (
    <div className="q-card" style={{ animationDelay: `${delay}ms` }}>
      ...
      {q.type === 'written' ? (
        <textarea
          rows={3} className="q-textarea" placeholder="Your answer..."
          value={textVal}
        ...
        <div className="q-opts">
          {q.opts.map((opt, i) => (
            <button
              key={i}
              className={`opt-btn${isSelected(i) ? (q.type === 'msq' ? ' multi-selected' : ' selected') : ''}`}
              onClick={() => handleOpt(i)}
            >
  ...
  return (
    <div className="score-ring-wrap">
      <svg width="120" height="120" viewBox="0 0 120 120">
        ...
        <circle
          cx="60" cy="60" r="52" fill="none" stroke="url(#rg)" strokeWidth="6"
          strokeLinecap="round"
        ...
        {s.screen === 'setup' && (
          <div className="screen">
            ...
            <div className="section-label">
              Filter by category <span className="section-hint">(optional — skip to see all {ALL_QUESTIONS.length})</span>
            </div>
            ...
            <div className={`card card-glow animate-up${bCodeErr ? ' shake' : ''}`}>
              <div className="field" style={{ marginBottom: 0 }}>
                ...
                <input
                  ...
                  value={bCode} onChange={e => setBCode(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && handleBEnter()}
                  className={bCodeErr ? 'input-error' : ''}
                />
                <p className="field-hint">
                  {bCodeErr
                    ? '✕ Wrong code. Try the name of the person who sent you this.'
                    : "Hint: it's the first name of the person who sent you this link."}
                </p>
              </div>
            </div>
        ...
        {s.screen === 'result' && (
          !canSeeResult ? (
            ...
            <div className="screen">
              ...
              <button
                className="btn btn-ghost" style={{ marginTop: '1rem' }}
